...
# キャプチャ突破
def BreakCaptcha():
    cmd = "convert captcha.jpg  -negate -morphology erode octagon:2 -negate -threshold 70% conv_captcha.jpg"
...
def main():
    ...
    # メニューを開く
    # url, data = SetData(1, resp)
    # resp, referrer = PostData(sess, url, referrer, sessID, data)
    
    
    # 空き状況照会
    # url, data = SetData(2, resp)
    # resp, referrer = PostData(sess, url, referrer, sessID, data)
    
    
    # 大分類選択
    # url, data = SetData(3, resp)
    # resp, referrer = PostData(sess, url, referrer, sessID, data)
    
    
    # 小分類選択
    url, data = SetData(4, resp)
    resp, referrer = PostData(sess, url, referrer, sessID, data)
    
    
    # 体育館選択
    url, data = SetData(5, resp)
    resp, referrer = PostData(sess, url, referrer, sessID, data)
    
    
    # 日付表示
    url, data = SetData(6, resp)
    resp, referrer = PostData(sess, url, referrer, sessID, data)
    
    # 日付選択
    url, data = SetData(7, resp)
    resp, referrer = PostData(sess, url, referrer, sessID, data)
    
    
    # 面数、人数確定
    url, data = SetData(8, resp)
    resp, referrer = PostData(sess, url, referrer, sessID, data)
    DownloadImage(sess, "./captcha.jpg")
    
    
    # 予約確定
    url, data = SetData(9, resp)
    resp. referrer = PostData(sess, url, referrer, sessID, data)
